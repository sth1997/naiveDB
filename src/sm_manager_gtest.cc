...
TEST_F(SM_ManagerTest, Test) {
    ...
    ASSERT_EQ(smm.CheckAttr("testRel", "attr1", found), OK_RC);
    ASSERT_EQ(found, false);
    ASSERT_EQ(smm.CreateTable("testRel", 2, attrs), OK_RC);
    ASSERT_EQ(smm.CheckRel("testRel", found), OK_RC);
    ASSERT_EQ(found, true);
    ASSERT_EQ(smm.CheckAttr("test", "attr1", found), OK_RC);
    ASSERT_EQ(found, false);
    ASSERT_EQ(smm.CheckAttr("testRel", "attr2", found), OK_RC);
    ASSERT_EQ(found, false);
    ASSERT_EQ(smm.CheckAttr("testRel", "attr1", found), OK_RC);
    ASSERT_EQ(found, true);
    ASSERT_EQ(smm.CreateTable("testRel", 2, attrs), SM_BADTABLEPARA);
    ...
    ASSERT_EQ(smm.IsAttrIndexed("testRel", "attr1", indexed), OK_RC);
    ASSERT_EQ(indexed, false);
    ASSERT_EQ(smm.IsAttrIndexed("testRel", "attr", indexed), OK_RC);
    ASSERT_EQ(indexed, false);
    ASSERT_EQ(smm.IsAttrIndexed("test", "attr1", indexed), OK_RC);
    ASSERT_EQ(indexed, false);
